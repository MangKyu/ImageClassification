...
class ImageClassification:
    ...
    def get_accuracy(self, file_name):
        answer = None
        folder_path = os.path.join('Files', 'Training')
        image_path = os.path.join(folder_path, file_name)  # , file_name)
        ...
        with tf.Session() as sess:
            ...
            photo_label, accuracy = labels[top_k[0]], predictions[top_k[0]]
            photo_label = photo_label.split("'")[1].split('\\')[0]

            dir_path = os.path.join(folder_path, 'photos')
            dir_list = os.listdir(dir_path)
            for photo_dir in dir_list:
                if photo_dir.lower() == photo_label.lower():
                    photo_label = photo_dir
                    break
            return photo_label, accuracy
...
if __name__ == "__main__":
    ...
    # photos should be on Files/Training/photos/* => with own directory classified by images
    image_classification.start_training()
    # Image should be on Files/Training/file_name.jpg
    file_name = 'test.jpg'
    uid, accuracy = image_classification.get_accuracy(file_name)
    print(uid, accuracy)
